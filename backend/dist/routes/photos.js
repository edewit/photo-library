...
const exifExtractor_1 = require("../utils/exifExtractor");
const thumbnailGenerator_1 = require("../utils/thumbnailGenerator");
const eventSuggester_1 = require("../utils/eventSuggester");
const fileOrganizer_1 = require("../utils/fileOrganizer");
const router = express_1.default.Router();
const uploadsDir = process.env.UPLOAD_PATH || '../uploads';
// Upload photos and suggest events
router.post('/upload', upload_1.upload.array('photos', 100), async (req, res) => {
    try {
        ...
        for (const file of req.files) {
            ...
            // Extract EXIF data
            const metadata = await (0, exifExtractor_1.extractExifData)(filePath, file.filename, file.originalname, file.size, file.mimetype);
            // Generate thumbnail
            const thumbnailPath = await (0, thumbnailGenerator_1.generateThumbnail)(filePath, file.filename, uploadsDir);
            uploadedPhotos.push({
        ...
        const eventSuggestions = (0, eventSuggester_1.suggestEvents)(uploadedPhotos);
        res.json({
...
router.post('/organize', async (req, res) => {
    try {
        ...
        for (const eventData of events) {
            ...
            const photoFilenames = eventData.photos.map((photo) => photo.filename);
            // Organize photos into event folder
            const { movedFiles } = await (0, fileOrganizer_1.organizePhotosIntoEventFolder)(eventData.name, photoFilenames, uploadsDir);
            ...
            for (const photoData of eventData.photos) {
                ...
                const photo = new Photo_1.Photo({
                    ...
                    filePath: movedFile ? (0, fileOrganizer_1.getEventPhotoPath)(eventData.name, photoData.filename) : photoData.filename,
                    thumbnailPath: (0, fileOrganizer_1.getEventThumbnailPath)(eventData.name, photoData.filename),
            ...
            if (eventPhotos.length > 0) {
                ...
                event.coverPhotoId = coverPhoto._id;
                await event.save();
            }
...
router.get('/:id', async (req, res) => {
    try {
        const photo = await Photo_1.Photo.findById(req.params.id);
        if (!photo) {
...
router.get('/thumbnail/:photoId', async (req, res) => {
    try {
        ...
        if (mongoose_1.default.Types.ObjectId.isValid(photoId)) {
            ...
            const filePath = path_1.default.join(uploadsDir, photo.thumbnailPath);
            await promises_1.default.access(filePath);
            return res.sendFile(path_1.default.resolve(filePath));
        }
        else {
            // It's a direct filename (legacy), serve from thumbnails folder
            const filePath = path_1.default.join(uploadsDir, 'thumbnails', photoId);
            await promises_1.default.access(filePath);
            return res.sendFile(path_1.default.resolve(filePath));
        }
    }
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        // Delete files
        const photoPath = path_1.default.join(uploadsDir, photo.filePath);
        const thumbnailPath = path_1.default.join(uploadsDir, photo.thumbnailPath);
        try {
        ...
        await Photo_1.Photo.findByIdAndDelete(req.params.id);
        // Update event photo count
        await Event_1.Event.findByIdAndUpdate(photo.eventId, {
            $inc: { photoCount: -1 }
        });
